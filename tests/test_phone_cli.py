...
def test_call_tools_not_loaded_for_diagnostics(calling_env, option):
    with patch("commbadge.phone.config.PhoneSettings.load") as load:
        if option == "--check":
            with patch("commbadge.live.connect_voice", new_callable=AsyncMock):
                assert main(["voice", option, "--env-file", str(calling_env)]) == 0
        else:
            with patch("commbadge.cli.MacAudio.driver") as driver:
...
def test_standalone_call_still_exits_and_reports_failures(calling_env, outcome, exit_code):
    with (
        patch("commbadge.phone.cli.AlsaAudio") as audio,
        patch(
            "commbadge.phone.client.call_contact",
            new_callable=AsyncMock,
            return_value={"status": outcome},
        ),
        patch("commbadge.live.connect_voice") as voice,
    ):
