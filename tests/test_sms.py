...
def test_send_posts_correct_text_sender_and_auth_without_claiming_delivery(monkeypatch):
    ...
    monkeypatch.setattr("combadge.sms.urlopen", open_request)
...
def test_network_or_invalid_response_is_unknown_and_never_resent(monkeypatch, failure):
    async def scenario():
        ...
        monkeypatch.setattr("combadge.sms.urlopen", opener)
        sms = client()
        first = await sms.send("alex", "Hello", "d1")
...
def test_cancelled_send_is_uncertain_even_in_new_delegation():
    async def scenario():
        sms = client()
        with patch("combadge.sms.asyncio.to_thread", side_effect=asyncio.CancelledError()) as send:
...
def test_cli_sms_needs_no_openai_or_sip_configuration(env_file, capsys, action):
    ...
    if action == "send":
        args += ["alex", "Hello"]
    with patch("combadge.cli.SmsClient") as cls:
...
def test_cli_unsuccessful_send_has_nonzero_exit(env_file, status):
    with patch(
        "combadge.cli.SmsClient.send", new_callable=AsyncMock, return_value={"status": status}
    ):
...
def test_voice_cli_enables_sms_automatically_with_explicit_override(
    ...
    with (
        ...
        patch("combadge.sms.urlopen") as request,
    ):
...
def test_voice_audio_check_does_not_load_configured_sms(env_file, monkeypatch):
    ...
    with (
        patch("combadge.live.connect_voice", new_callable=AsyncMock) as connect,
        patch("combadge.cli.SmsSettings.load") as load_sms,
    ):
